...
namespace LowPolyWater
{
    public class LowPolyWater : MonoBehaviour
    {
        ...
        public Vector3[] NearestVertexTo(Vector3 point)
        {
            ...
            var minSecondDistanceSqr = Mathf.Infinity;
            var secondNearestVertex = Vector3.zero;

            var minThirdDistanceSqr = Mathf.Infinity;
            var thirdNearestVertex = Vector3.zero;
            ...
            foreach (var vertex in mesh.vertices)
            {
                ...
                {
                    minSecondDistanceSqr = minDistanceSqr;
                    secondNearestVertex = nearestVertex;

                    minDistanceSqr = distSqr;
                    nearestVertex = vertex;
                }
                else if (distSqr < minSecondDistanceSqr)
                {
                    minThirdDistanceSqr = minSecondDistanceSqr;
                    thirdNearestVertex = secondNearestVertex;

                    minSecondDistanceSqr = distSqr;
                    secondNearestVertex = vertex;
                }
                else if (distSqr < minThirdDistanceSqr)
                {
                    minThirdDistanceSqr = distSqr;
                    thirdNearestVertex = vertex;
                }
            }
            ...
            return new[]
            {
                ...
                transform.TransformPoint(secondNearestVertex),
